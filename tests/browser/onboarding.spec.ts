...
test('setup takes one small decision at a time and can finish without service keys', async ({ page }, info) => {
	...
	await page.screenshot({ path: info.outputPath('setup-welcome.png') });
	await page.keyboard.press('Escape');
	await expect(dialog).toBeVisible();
	await dialog.getByRole('button', { name: 'Get started' }).click();
	await expect(dialog.getByRole('heading', { name: 'Choose your avatar' })).toBeVisible();
	await page.screenshot({ path: info.outputPath('setup-avatar.png') });
	...
	await page.screenshot({ path: info.outputPath('setup-chat.png') });
