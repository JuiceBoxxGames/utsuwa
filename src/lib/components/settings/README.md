...
- `src/lib/styles/app-controls.css` owns input, dropdown, checkbox, radio, range, and dialog recipes, plus the field layout helpers. `src/routes/app/settings/settings-page.css` owns the settings column, page header, and the `setting-row` family. Use `settings-field`, `settings-range`, and `settings-label`. Add `use:rangeProgress={value}` to range inputs so the track follows keyboard edits and programmatic resets.
- `omnivoice/` holds the OmniVoice voice picker and clone dialog. Calls to the OmniVoice proxy go through `$lib/services/tts/omnivoice-client.ts`.
